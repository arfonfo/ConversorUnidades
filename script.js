...
document.getElementById('convert').addEventListener('click', () =>{
  ...
  if(fromCategory && fromUnit && toUnit && conversionFactors[fromCategory] && conversionFactors[fromCategory][fromUnit] && conversionFactors[fromCategory][fromUnit][toUnit]){
    ...
    const convertedValue = conversionFactor * inputValue;
    results.innerHTML = `${inputValue} ${fromUnit} = ${convertedValue.toFixed(2)} ${toUnit}`;
  } else {
    results.innerHTML = 'Conversión inválida';
  }
});
...
document.getElementById('reset').addEventListener('click', () => {
  ...
  selectedDropdowns.forEach(selectedDropdown => {
    selectedDropdown.textContent = "Elige";
  });
